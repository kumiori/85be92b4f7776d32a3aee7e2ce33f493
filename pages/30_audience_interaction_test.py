...
def _render_question(question: Question, answers: dict[str, Any]) -> None:
    with st.container(border=True):
        ...
        if question.qtype == "single":
            options = ["Select an option"] + (question.options or [])
            existing = answers.get(question.id)
            idx = options.index(existing) if existing in options else 0
            selected = st.radio(
                " ", options, index=idx, key=widget_key, label_visibility="collapsed"
            )
            answers[question.id] = "" if selected == "Select an option" else selected

        elif question.qtype == "multi":
            existing = (
                answers.get(question.id)
                if isinstance(answers.get(question.id), list)
                else []
            )
            ...
            text = st.text_input(
                "One word (max 24 chars)",
                value=existing,
                max_chars=24,
                key=widget_key,
                help="Your word will be lowercased for aggregate stats.",
            )
            answers[question.id] = text


def _render_aggregates(rows: list[dict[str, Any]], questions: list[Question]) -> None:
    st.subheader("Aggregated Results (Current Session)")
    st.caption("Anonymised counts for submitted responses.")
...
def main() -> None:
    ...
    microcopy("This takes ~2 minutes. Answers are aggregated after you submit.")
    ...
    selected_questions = select_questions(
        ...
        session_id=selected_session_label,
    )
    ...
    if total == 0:
        st.warning(
            f"No active catalogue questions are linked to session `{selected_session_label}`."
        )
        return
    ...
    st.caption(
        f"Depth {depth} shows {question_count_for_depth(depth)} questions. "
        f"Selection is stable for this session and participant."
    )
...
.interaction-footer {
  ...
  z-index: 20;
    ...
    with back_col:
        if st.button("Back", use_container_width=True, disabled=index == 0):
            st.session_state["interaction_index"] = max(0, index - 1)
            st.rerun()
    with next_col:
        next_disabled = index >= total - 1 or not _is_answer_valid(
    ...
    with submit_col:
        ...
        do_submit = st.button(
            "Submit", type="primary", use_container_width=True, disabled=submit_disabled
        )
    ...
    try:
        ...
        st.error(
            "Interaction storage is Notion-only and is not configured correctly. "
            f"Details: {exc}"
        )
        st.stop()
    ...
    if do_submit:
        for q in selected_questions:
            ...
            if not _is_answer_valid(q, normalized):
                ...
                st.stop()
        for page_index, q in enumerate(selected_questions, start=1):
            normalized_value = _normalize_answer_for_storage(q, answers.get(q.id))
        ...
        if player_id:
            ok, err = touch_player_presence(
                str(player_id),
                page="interaction_submit",
                session_slug=selected_session_id,
            )
            if not ok:
                st.toast(f"Presence update failed: {err}", icon="⚠️")
        st.session_state["interaction_submitted"] = True
        st.success("Thanks. Your responses were saved. Aggregates are now visible.")
        st.rerun()
    ...
    if st.session_state.get("interaction_submitted"):
        ...
        st.info("Aggregates unlock after submission to reduce social influence.")
