...
def build_credentials_pdf(
    *,
    ...
    purpose_line = "Purpose: placeholder intention/destination."

    content = [
        # ("Emoji String", spaced_emoji or "-", emoji_font),
        # ("Emoji-4", spaced_emoji4, emoji_font),
        # ("Emoji-6", spaced_emoji6, emoji_font),
        ("Passphrase", phrase_wrapped or "-", text_font),
    ]

    y = margin + header_h * 2 + 22
    ...
    draw.text((margin, y), "ICEICEBABY", fill=(15, 48, 87), font=title_font)
    y += header_h
    draw.text((margin, y), title, fill=(15, 48, 87), font=text_font)
    y += 30
    draw.text((margin, y), purpose_line, fill=(36, 52, 71), font=text_font)
    y += header_h + 16
    ...
    y += 2
    def _draw_token_row(title: str, token_symbols: List[str], palette_offset: int = 0) -> None:
        nonlocal y
        if not token_symbols:
            return
        x = value_x
        total = len(token_symbols)
        draw.text((label_x, y), title, fill=(36, 52, 71), font=text_font)
        for idx, symbol in enumerate(token_symbols):
            if x + 72 > width - margin:
                x = value_x
                y += 44
            bg = chip_palette[(idx + palette_offset) % len(chip_palette)]
            ...
            # Reverse numbering: first symbol gets N, last symbol gets 1.
            draw.text(
                (x + 44, y + 9),
                str(total - idx),
                fill=(70, 70, 70),
    ...
    # Token section order: emoji-4, emoji-6, then full emoji.
    _draw_token_row("Emoji-4 tokens :", suffix4_symbols, 2)
    _draw_token_row("Emoji-6 tokens :", suffix6_symbols, 3)
    _draw_token_row("Emoji tokens :", symbols, 0)

    draw.line([(margin, y), (width - margin, y)], fill="black", width=1)
    y += 14
    draw.text(
        (margin, y), "Keep this document in a safe place.", fill="black", font=text_font
    )
