...
def resolve_question_set_bundle(
    *,
    ...
    if entry is None and resolved_text_id:
        entry = next(
            (
                item
                for item in _REGISTRY
                if resolved_text_id in item.text_ids
            ),
            None,
        )
    if entry is None:
    ...
    return ResolvedQuestionSetBundle(
        event_slug=str((context.get("event_slug") if config else "") or entry.event_slug),
        session_code=str((context.get("session_code") if config else "") or entry.session_code),
        ...
        event_specific_question_ids=tuple(event_specific_question_ids(entry.question_set)),
...
def registry_validation_errors() -> list[str]:
    ...
    for entry in _REGISTRY:
        if entry.question_set_id in seen_ids:
            errors.append(f"Duplicate registered question_set_id: {entry.question_set_id}")
        ...
        rows.append(
            {
                "event_slug": str((context.get("event_slug") if config else "") or entry.event_slug),
                "session_code": entry.session_code,
                "text_id": (context.get("text_id") if config else "") or entry.text_ids[0],
                "question_set_id": entry.question_set_id,
                "schema_id": entry.schema_id,
                "question_set_module": entry.question_set.source_module,
                "question_ids": list(question_ids(entry.question_set)),
                "shared_question_ids": list(shared_question_ids(entry.question_set)),
                "event_specific_question_ids": list(event_specific_question_ids(entry.question_set)),
